...
// This deliberately rides the same PriceBar universe the GP chart
// fills and the nightly cron keeps warm: no extra upstream, no Yahoo,
// no paid feed, and nothing network-bound on the request path. The
// trade-off is honest and the UI states it — this is close-to-close
// over the universe we track, not a live whole-market tape.
export async function getMovers({ limit = 10 } = {}) {
  const n = Math.min(25, Math.max(1, Math.trunc(Number(limit)) || 10));

  // One bounded query. Twelve calendar days always spans at least two
  // trading sessions even across a long holiday weekend, so every
  // ticker that's current has a prior close to diff against. Rows come
  // back newest-first within each ticker.
  const since = new Date(Date.now() - 12 * 86400_000);
  ...
  for (const r of rows) {
    ...
    if (!acc) byTicker.set(r.ticker, [r]);
    else if (acc.length < 2) acc.push(r);
  }

  // Reference day = the freshest bar date anywhere in the universe. We
  // only rank tickers whose latest bar lands on that day, so a ticker
  // the cron happened to miss can't show a stale multi-day move
  // mislabeled as today's.
  let refDate = 0;
  for (const [, bars] of byTicker) {
    const t = bars[0].date.getTime();
    if (t > refDate) refDate = t;
  }
  ...
  for (const [ticker, bars] of byTicker) {
    ...
    const [cur, prev] = bars;
    if (cur.date.getTime() !== refDate) continue;
    if (cur.close == null || prev.close == null || prev.close <= 0) continue;
  ...
  return {
    asOf: refDate ? new Date(refDate).toISOString().slice(0, 10) : null,
    universe: byTicker.size,
    ranked: moves.length,
    gainers,
    losers,
  };
