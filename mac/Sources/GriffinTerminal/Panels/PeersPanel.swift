    ...
    var body: some View {
        PanelState(state: state,
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                if (p.count ?? 0) == 0 {
                    Text("Finnhub has no peer set for \(p.ticker ?? ticker.uppercased()) — common for ETFs, funds, and thinly-covered names. Showing \(p.ticker ?? ticker.uppercased()) alone.")
                        .font(Term.mono(10))
                ...
                Divider().overlay(Term.border)
                Text("Finnhub peer set · fundamentals are a ~15m snapshot · focus row highlighted")
                    .font(Term.mono(9))
                    .foregroundStyle(Term.fgMuted)
                    .padding(.horizontal, 10)
                    .padding(.vertical, 5)
