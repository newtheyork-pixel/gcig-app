...
- `Core/Parser.swift` is a PORT of `client/src/terminal/parser.js`;
  ...
  HOOT (aliases SQUAWK, DESK) must exist in all three lists: web
  `registry.js`, server `KNOWN_FUNCTIONS`, and this Registry. Missing
  it from the web list made `HOOT` parse as a ticker and open DES.
- The desk squawk box is `GET /ws/hoot` (not under `/api`). Local
  Vite must proxy `/ws` with `ws: true`; production CSP must list
  `wss://gcig-api.onrender.com` and `microphone=(self)` — `https://`
  does not cover WebSockets, and `microphone=()` bans the mic on the
  page that asked. The `ws` server runs `perMessageDeflate: false`
  because URLSessionWebSocketTask drops frames once it is on.
  SQUAWK and DESK are their own `KNOWN_FUNCTIONS` rows (same pattern as
