...
enum Fmt {
    static func money(_ v: Double) -> String {
        ...
        f.numberStyle = .currency; f.maximumFractionDigits = 0; f.currencyCode = "USD"
...
final class Session: ObservableObject {
    ...
    func get<T: Decodable>(_ path: String, as: T.Type) async throws -> T {
        ...
        let (d, resp) = try await URLSession.shared.data(for: r)
        // Only a 401 ends a session. Everything else keeps the login and
        // shows an error, which is the rule the web and Mac clients learned
        // the hard way: a 429 or a cold-start 502 used to log people out.
    ...
    private static func keychainWrite(_ t: String) {
        keychainDelete()
        SecItemAdd(query([kSecValueData as String: Data(t.utf8),
                          kSecAttrAccessible as String: kSecAttrAccessibleAfterFirstUnlock]) as CFDictionary, nil)
    }
